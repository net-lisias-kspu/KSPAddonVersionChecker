...
namespace KSP_AVC
{
    public class VersionInfo : IComparable
    {
        ...
        public override string ToString()
        {
            ...
            if (this.Build != 0)
            {
                return String.Format("{0}.{1}.{2}.{3}", this.Major, this.Minor, this.Patch, this.Build).Replace("-1", "*");
            }
            return this.Patch != 0 ? String.Format("{0}.{1}.{2}", this.Major, this.Minor, this.Patch).Replace("-1", "*") : String.Format("{0}.{1}", this.Major, this.Minor).Replace("-1", "*");
        }
